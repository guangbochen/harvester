...
func RegisterBackup(ctx context.Context, management *config.Management, opts config.Options) error {
	...
	vmBackups.OnChange(ctx, backupControllerName, vmBackupController.OnChanged)
...
// OnChanged handles vm backup object on change
func (h *Handler) OnChanged(key string, vmBackup *harvesterapiv1.VirtualMachineBackup) (*harvesterapiv1.VirtualMachineBackup, error) {
...
func (h *Handler) createContent(vmBackup *harvesterapiv1.VirtualMachineBackup, source *kv1.VirtualMachine) error {
	contentName := getVMBackupContentName(vmBackup)
	logrus.Infof("create vm backup content %s/%s", vmBackup.Namespace, contentName)

	sourceVolumes := source.Spec.Template.Spec.Volumes
	...
	for volumeName, pvcName := range volumeToPvcMappings(sourceVolumes) {
		...
		vb := harvesterapiv1.VolumeBackup{
			...
			PersistentVolumeClaim: harvesterapiv1.PersistentVolumeClaimSpec{
				TypeMeta: pvcCpy.TypeMeta,
				ObjectMeta: metav1.ObjectMeta{
					Name:      pvcCpy.ObjectMeta.Name,
					Namespace: pvcCpy.ObjectMeta.Namespace,
				},
				Spec: pvcCpy.Spec,
			},
	...
	content := &harvesterapiv1.VirtualMachineBackupContent{
		...
		Spec: harvesterapiv1.VirtualMachineBackupContentSpec{
			...
			Source: harvesterapiv1.SourceSpec{
				TypeMeta: vmCpy.TypeMeta,
				ObjectMeta: metav1.ObjectMeta{
					Name:      vmCpy.ObjectMeta.Name,
					Namespace: vmCpy.ObjectMeta.Namespace,
				},
				VirtualMachineSpec: &vmCpy.Spec,
			},
