...
func (h *RestoreHandler) RestoreOnChanged(key string, restore *harvesterapiv1.VirtualMachineRestore) (*harvesterapiv1.VirtualMachineRestore, error) {
	...
	if err != nil {
		return nil, h.doUpdateError(restore, restoreCpy, fmt.Errorf("error reconciling VolumeRestores"))
	}
	...
	if !updated {
		...
		if err != nil {
			return nil, h.doUpdateError(restore, restoreCpy, fmt.Errorf("error checking target ready, err:%s", err.Error()))
		}
		...
		if ready {
			...
			if err != nil {
				return nil, h.doUpdateError(restore, restoreCpy, fmt.Errorf("error reconciling target, err:%s", err.Error()))
			}

			if !updated {
				if err = target.Cleanup(); err != nil {
					return nil, h.doUpdateError(restore, restoreCpy, fmt.Errorf("error cleaning up, err:%s", err.Error()))
				}
		...
		if restoreCpy.Annotations[restoreNameAnnotation] != target.vm.Name && target.vm != nil {
			restoreCpy.Annotations[restoreNameAnnotation] = target.vm.Name
		}
...
func (h *RestoreHandler) ReconcileVMTemplate(vm *kv1.VirtualMachineSpec,
	...
	fmt.Println("reconcile vm template")
