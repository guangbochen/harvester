...
func RegisterRestore(ctx context.Context, management *config.Management, opts config.Options) error {
	...
	handler := &RestoreHandler{
		...
		recorder:          management.NewRecorder(restoreControllerName, allNamespaces, ""),
	}
...
func (h *RestoreHandler) RestoreOnChanged(key string, restore *harvesterapiv1.VirtualMachineRestore) (*harvesterapiv1.VirtualMachineRestore, error) {
	...
	logrus.Infof("updating VirtualMachineRestore %s/%s", restore.Namespace, restore.Name)
	complete := false
	...
	restoreCpy.Status.Complete = &complete
	...
	content, err := h.getBackupContent(restore, target.UID(), restore.Spec.NewVM)
	if err != nil {
...
func (h *RestoreHandler) getTarget(vmRestore *harvesterapiv1.VirtualMachineRestore) (restoreTarget, error) {
	...
	for _, vb := range content.Spec.VolumeBackups {
		...
		if !found {
			...
			vr := harvesterapiv1.VolumeRestore{
				...
				VolumeBackupName:          *vb.VolumeBackupName,
			}
			restores = append(restores, vr)
		}
	}
...
func (h *RestoreHandler) createRestorePVC(vmRestore *harvesterapiv1.VirtualMachineRestore, target restoreTarget,
	...
	if volumeBackup.VolumeBackupName == nil {
		logrus.Errorf("volumeSnapshot name missing %+v", volumeBackup)
	...
	pvc.Spec.DataSource = &corev1.TypedLocalObjectReference{
		...
		Name:     *volumeBackup.VolumeBackupName,
	}
	pvc.Spec.VolumeName = ""
